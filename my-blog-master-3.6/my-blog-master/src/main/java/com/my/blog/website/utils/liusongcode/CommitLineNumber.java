...
public class CommitLineNumber {
    ...
    public int getConstAllNum() {
        return constAllNum;
    }

    public int getStaticAllNum() {
        return staticAllNum;
    }

   static  private int lineNum=0,blankNum=0,commentNum=0,codeNum=0,constAllNum=0,staticAllNum=0;
    ...
    public  StringBuffer showResult(){
        ...
        commentNum+=commentLines;
        constAllNum+=constNum;
        staticAllNum+=staticNum;
        res.append("注释行数:"+commentLines+"  注释行数比例"+nt.format(commentPercent)+"\r\n");
        res.append("const常量个数："+constNum+"\r\n");
        res.append("static静态变量个数："+staticNum+"\r\n");
        return res;
    }
}
